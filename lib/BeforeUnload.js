(function (root, factory) {
    ...
    /**
     * Calls the callback if there is no blocking conditions, or if
     * the user confirmed the beforeUnload prompt.
     *
     * Unless preserveHandlers is set to true, the handlers will be
     * unregistered before calling the callback.
     */
    BeforeUnload.prototype.confirmedIfNecessary = function (callback, preserveHandlers) {
        var conditions = this.check();

        if (!conditions || window.confirm(typeof conditions === 'string' ? conditions : this.message)) {
            if (!preserveHandlers) { this.unregister(); }
            callback();
        }
    };
